...
const Select = ({options = [], value = null, onChange, placeholder="انتخاب", searchInputPlaceholder = "جستجو", isMultiple = false, isClearable = true, isSearchable = true, isDisabled = false, loading = false, menuIsOpen = false, noOptionsMessage = "No results found"}) => {
    ...
    return (
        <div className="relative w-full" ref={ref}>
            <div onFocus={toggle} onKeyDown={pressEnter} onClick={toggle} className={`flex items-stretch w-full h-full text-sm text-gray-500 border border-gray-300 rounded shadow-sm ring-2 ring-blue-500 transition duration-300 ${isDisabled ? 'bg-gray-200' : 'bg-white hover:border-gray-400'} ${!open ? 'ring-opacity-0' : ''}`}>
                <div className="w-full pl-2.5 py-2 pr-2 text-sm flex flex-nowrap gap-1 overflow-auto whitespace-nowrap hide-scroll">
                    {!isMultiple ? (
                            ...
                            {value === null ? (
                                ...
                                value.map((item, index) => (
                                    <div className="bg-gray-200 rounded-sm flex space-x-1" key={index}>
                                        ...
                                        {!isDisabled && (
                                            <div onClick={e => removeOptionOnValue(e, item)} className={`flex items-center px-1 cursor-pointer rounded-r-sm  hover:text-red-600`}>
                                                <ClearIcon />
                                            </div>
                                        )}
                                    </div>
